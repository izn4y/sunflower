...
router.post("/sendmail", (req, res) => {
  ...
  if (!name || !service || !email || !message)
    formError.push({ empty_fileds: "Merci de remplir tous les champs" });
  if (!arrayService.includes(service))
    formError.push({ empty_fileds: "Merci de sélectionner un sujet" });
  ...
  if (formError.length > 0) {
    ...
    MailSender.sendVerificationMail(name, email, service, message)
      ...
      .catch((error) => {
        UtileLog.logWritingError(
          error,
          true,
          MailSender.sendVerificationMail.name
        );
